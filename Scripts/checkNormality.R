...
# RN-Transformation
for (i in 2:ncol(dat)){
dat[,i] <- rntransform(dat[,i])}

# Test for Normality
norm <- data.frame(var=colnames(dat)[2:ncol(dat)])
norm$pval <- NA
for(i in 1:nrow(norm)){
name <- as.character(norm[i,1])
norm[i,2] <- shapiro.test(as.numeric(as.character(dat[,name])))$p.val
}
nnorm <- norm[norm[,2] < 0.05,]


print("Not normally distributed phenotypes:")
print(nnorm)
